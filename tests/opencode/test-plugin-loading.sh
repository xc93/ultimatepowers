#!/usr/bin/env bash
# Test: Plugin Loading
# Verifies that the ultimatepowers plugin loads correctly in OpenCode
set -euo pipefail
...
plugin_link="$OPENCODE_CONFIG_DIR/plugins/ultimatepowers.js"
...
skill_count=$(find "$ULTIMATEPOWERS_SKILLS_DIR" -name "SKILL.md" | wc -l)
if [ "$skill_count" -gt 0 ]; then
    echo "  [PASS] Found $skill_count skills"
else
    echo "  [FAIL] No skills found in $ULTIMATEPOWERS_SKILLS_DIR"
    exit 1
fi
...
if [ -f "$ULTIMATEPOWERS_SKILLS_DIR/using-superpowers/SKILL.md" ]; then
...
if node --check "$ULTIMATEPOWERS_PLUGIN_FILE" 2>/dev/null; then
...
if grep -q 'configDir}/skills/ultimatepowers/' "$ULTIMATEPOWERS_PLUGIN_FILE"; then
