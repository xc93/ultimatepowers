...
run_present_file_check() {
    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$ULTIMATEPOWERS_PLUGIN_FILE" present
}

run_missing_file_check() {
    mv "$ULTIMATEPOWERS_SKILLS_DIR/using-superpowers/SKILL.md" "$TEST_HOME/using-superpowers.SKILL.md.bak"

    node "$SCRIPT_DIR/test-bootstrap-caching.mjs" "$ULTIMATEPOWERS_PLUGIN_FILE" missing
}
