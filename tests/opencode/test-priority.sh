...
# 1. Create in ultimatepowers location (lowest priority)
mkdir -p "$ULTIMATEPOWERS_SKILLS_DIR/priority-test"
cat > "$ULTIMATEPOWERS_SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Ultimatepowers version of priority test skill
---
# Priority Test Skill (Ultimatepowers Version)

This is the ULTIMATEPOWERS version of the priority test skill.

PRIORITY_MARKER_ULTIMATEPOWERS_VERSION
EOF
...
if [ -f "$ULTIMATEPOWERS_SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] Ultimatepowers version exists"
else
    echo "  [FAIL] Ultimatepowers version missing"
    exit 1
fi
...
# Test 2: Document personal vs bundled ultimatepowers priority
echo ""
echo "Test 2: Documenting personal vs ultimatepowers priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled ultimatepowers version instead of personal version"

# Test 3: Document project vs bundled ultimatepowers priority
echo ""
echo "Test 3: Documenting project vs personal/ultimatepowers priority..."
...
describe_priority_result \
    ...
    "Current OpenCode behavior loaded bundled ultimatepowers version instead of project version"

# Test 4: Test a non-colliding bundled ultimatepowers skill is still available
echo ""
echo "Test 4: Testing non-colliding ultimatepowers skill remains available..."

mkdir -p "$ULTIMATEPOWERS_SKILLS_DIR/ultimatepowers-only-test"
cat > "$ULTIMATEPOWERS_SKILLS_DIR/ultimatepowers-only-test/SKILL.md" <<'EOF'
---
name: ultimatepowers-only-test
description: Ultimatepowers-only priority test skill
---
# Ultimatepowers Only Test Skill

PRIORITY_MARKER_ULTIMATEPOWERS_ONLY_VERSION
EOF

run_opencode output "$TEST_HOME/test-project" "Call the skill tool with name \"ultimatepowers-only-test\". Show the exact content including any PRIORITY_MARKER text."
assert_contains "$output" "PRIORITY_MARKER_ULTIMATEPOWERS_ONLY_VERSION" "Non-colliding ultimatepowers skill is still registered"
